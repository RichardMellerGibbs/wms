...
router.post('/', function (req, res) {
  ...
  if (req.body.customer !== true && req.body.customer !== false) {
    return res.json({
      success: false,
      message: 'No customer identifer specified',
    });
  } else {
  ...
  if (req.body.admin !== true && req.body.admin !== false) {
    return res.json({
      success: false,
      message: 'No admin identifier specified',
    });
  } else {
...
router.put('/:user_id', authMiddle.isAuthenticated, function (req, res, next) {
  logger.info(
    'Processing request to update a single user specified by id %s',
    req.params.user_id
  );
...
router.get('/checkname/:username', authMiddle.isAuthenticated, function (
  req,
  res
) {
...
router.get('/:user_id', authMiddle.isAuthenticated, function (req, res) {
  logger.info(
    'Processing request to get a single user specified by id %s',
    req.params.user_id
  );
...
router.delete('/:user_id', authMiddle.isAuthenticated, function (req, res) {
  logger.info(
    'Processing request to delete a single user specified by id %s',
    req.params.user_id
  );
