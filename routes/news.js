...
router.get('/:news_id', function (req, res) {
  logger.info(
    'Processing request to get a single news article specified by id %s',
    req.params.news_id
  );
  ...
  models.News.findById(req.params.news_id, function (err, news) {
    ...
    if (!news) {
      ...
      responses.handleError(
        new Error('Error finding the news id ' + req.params.news_id),
        req,
        res
      );
      return res.json({ success: false, message: 'Internal server error' });
  ...
  if (req.body.articleUrlDescription) {
    if (!req.body.articleUrl) {
      return res.json({
        success: false,
        message:
          'An articleUrl must be specified if a URL description is present',
      });
    }
  }
...
router.delete('/:news_id', authMiddle.isAuthenticated, function (req, res) {
  logger.info(
    'Processing request to delete news specified by id %s',
    req.params.news_id
  );
